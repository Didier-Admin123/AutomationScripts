...
<# If ($members -contains $user -or $members1 -contains $user -or $members2 -contains $user ) {
	...
	#Begin Statement 2)
	If ($User -eq $Null) {
	echo "<div style=float:left><h4>$username was not found in Active Directory </h4><h4 style=color:red>(Remember to type the LogonName or SamAccountName)</h4></div>"

	#End Statement 2)
	}Else{

	Get-ADUser $username -Properties enabled | disable-adaccount
