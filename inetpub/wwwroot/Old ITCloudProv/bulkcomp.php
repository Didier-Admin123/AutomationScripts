...
<div class="img" style="width:100px">
 <img src="computers.ico" alt="Trolltunga Norway" width="200" height="180">
  </div>
...
<h2>Add/Remove From Bulk Comp<h3></div> 
	<form method="post" enctype="multipart/form-data"/>
 Type AD Group Name
   <input type="text" name="username" required id="username" maxlength="20" />
	 <br><input type="radio" name="group" value="add">Add Group to Comp<br>
	  <input type="radio" name="group" value="remove">Remove Group From Comp
	   <br></br>
 Select CSV file:
...
if ($_POST['group'] == "add") {
	...
	$psScriptPath = "C:\inetpub\psroot\bulkadcomp.ps1";  //the CSV is a variable if you chance that you must chance whats in the parameter in the powershell script 
...
if ($_POST['group'] == "remove") {
	...
	$file=$_FILES['file']['name'];
	$username = trim($_POST["username"]);
	$psScriptPath = "C:\inetpub\psroot\bulkrmcomp.ps1";  //the CSV is a variable if you chance that you must chance whats in the parameter in the powershell script 
    $query = shell_exec("powershell -command $psScriptPath -csv '$file' -group '$username'< NUL");
    echo $query;
